...
# datos1977 <- datos1977[, -grep(pattern = c("CER"), colnames(datos1977))]
...
for (prov in seq_len(nrow(datos1977))) {
  ...
  if (prov==15) { datos1977[prov, 12] <- 23.866  }
  #
  ...
  library(stringr)
  names(selec) <- gsub("\\d", "", names(selec)) # substr(names(selec), 1, nchar(names(selec))-1)
...
```{r Método Sainte-Lague }

library(data.table)
borrar <- NULL
for (prov in seq_len(nrow(datos2019))) {
  # Cota del 3%
  corte <- 100 * colSums(datos2019[prov, 10:ncol(datos2019)]) / sum(datos2019$`Votos válidos`[prov])
  partidos <- colnames(datos2019[, 10:ncol(datos2019)])[corte > 3]
  votos <- datos2019[prov, 10:ncol(datos2019)][corte > 3]
  # Asientos por circunscripción
