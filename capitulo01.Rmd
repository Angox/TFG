...
library(data.table)
borrar <- NULL 
for (prov in 1:nrow(datos2019)) {
  

# Cota del 3%
corte <- 100*colSums(datos2019[prov,10:75])/sum(datos2019$`Votos válidos`[prov])
partidos <- colnames(datos2019[,10:75])[corte>3]
votos <- datos2019[prov,10:75][corte>3]
# Asientos por circunscripción
n_asientos <- datos2019$asientos[prov]
s <- 1:n_asientos
...
library(stringr)
names(selec) <- substr(names(selec), 1, nchar(names(selec))-1)
...
names(borrar2)
as.data.table(aggregate(as.numeric(borrar2),by=list(names(borrar2)),FUN=sum))[order(-x)]
