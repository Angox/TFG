...
ggplot(data = dis1, mapping = aes(Escannos, Disproporcionalidad)) +
  ...
  labs(title = "Número de Escaños a repartir", subtitle = "Disproporcionalidad según D`Hondt", caption = "Angel Gonzalez",x="Número de Escaños")
...
asientos_dhondt <- function(partidos, votos,
  ...
  # Escaños
  div <- div <- c(1, (10*(2:n_escannos)-5)/7)
  G <- sapply(div, function(x) votos / x)
  cuota <- G[order(G, decreasing = T)][n_escannos]
  W <- (G >= cuota)
  seats <- rowSums(W)
